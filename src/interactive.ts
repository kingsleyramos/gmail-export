...
function banner(): void {
    ...
    console.log(theme.title('  ║') + chalk.bold.white('           📧  Gmail Export  📧                      ') + theme.title('║'));
    console.log(theme.title('  ║') + theme.subtitle('        Export your emails to CSV easily              ') + theme.title('║'));
    console.log(theme.title('  ╚══════════════════════════════════════════════════════╝'));
    console.log('\n');
}
...
export async function runInteractiveSetup(): Promise<ExportConfig> {
    ...
    if (!fs.existsSync(defaultCredsPath)) {
        console.log(theme.warning('  ⚠️  No credentials.json found in the current directory.'));
        console.log(theme.dim('     Run with --setup to see the Gmail API setup guide.\n'));
        ...
        if (!proceed) {
            console.log(theme.info('\n  Run ') + theme.highlight('npm run setup') + theme.info(' to see the setup guide.\n'));
            process.exit(0);
        }
    }
    ...
    if (limitChoice === 'custom') {
        const customLimit = await input({
            ...
            validate: (val) => {
                ...
                return !isNaN(n) && n >= 0 ? true : 'Please enter a valid number';
            },
        });
    ...
    const limitDisplay = config.maxMessages === 0 ? 'Unlimited' : config.maxMessages.toLocaleString();
    ...
    if (fieldChoice === 'custom') {
        ...
        const categoryOrder = ['sender', 'recipient', 'content', 'metadata', 'attachments'];

        const choices: Array<{value: string; name: string; checked: boolean} | Separator> = [];

        for (const category of categoryOrder) {
            const categoryFields = FIELD_DEFINITIONS.filter((f) => f.category === category);
            const icon = categoryIcons[category] ?? '●';
            const categoryName = category.charAt(0).toUpperCase() + category.slice(1);

            choices.push(new Separator(theme.dim(`\n  ${icon} ${categoryName}`)));
    ...
    if (config.fields?.includes('body_text') || config.fields?.includes('body_html')) {
        console.log('');
        const bodyLimit = await select({
            ...
            choices: [
                ...
                {value: '50000', name: `50,000 chars ${theme.dim('(full content)')}`},
                {value: '0', name: `No limit ${theme.dim('(warning: very large files)')}`},
            ],
        });
    ...
    if (splitChoice === 'custom') {
        const customSize = await input({
            ...
            validate: (val) => {
                const n = parseInt(val, 10);
                return !isNaN(n) && n > 0 ? true : 'Please enter a valid number greater than 0';
            },
        });
        config.maxBytesPerFile = parseInt(customSize, 10) * 1024 * 1024;
    } else if (splitChoice === '0') {
        config.maxBytesPerFile = 0; // No splitting
    } else {
        config.maxBytesPerFile = parseInt(splitChoice, 10) * 1024 * 1024;
    }
    ...
    if (config.maxBytesPerFile && config.maxBytesPerFile > 0) {
        const sizeMB = Math.round(config.maxBytesPerFile / (1024 * 1024));
        console.log(theme.dim(`  Split at: ${sizeMB} MB per file`));
        console.log(theme.dim(`  Note: Files will be named ${config.outputPrefix}_*_part001.csv, etc.`));
    }

    // ═══════════════════════════════════════════════════════════════════
    // STEP 5: Save & Confirm
    // ═══════════════════════════════════════════════════════════════════
    stepHeader(5, 5, 'Confirm & Save');
    ...
    // Summary
    const splitDisplay = config.maxBytesPerFile && config.maxBytesPerFile > 0 
        ? `${Math.round(config.maxBytesPerFile / (1024 * 1024))} MB` 
        : 'No';
    
    console.log('\n' + theme.title('┌──────────────────────────────────────────────────────┐'));
    console.log(theme.title('│') + chalk.bold.white('              📋 Export Summary                       ') + theme.title('│'));
    console.log(theme.title('├──────────────────────────────────────────────────────┤'));
    console.log(theme.title('│') + `  ${theme.dim('Query:')}      ${theme.value(config.query ?? '')}`.padEnd(63) + theme.title('│'));
    console.log(theme.title('│') + `  ${theme.dim('Limit:')}      ${theme.value(config.maxMessages === 0 ? 'Unlimited' : String(config.maxMessages))}`.padEnd(63) + theme.title('│'));
    console.log(theme.title('│') + `  ${theme.dim('Fields:')}     ${theme.value(String(config.fields?.length ?? 0) + ' selected')}`.padEnd(63) + theme.title('│'));
    console.log(theme.title('│') + `  ${theme.dim('Output:')}     ${theme.value(config.outputDir + '/')}`.padEnd(63) + theme.title('│'));
    console.log(theme.title('│') + `  ${theme.dim('Splitting:')}  ${theme.value(splitDisplay)}`.padEnd(63) + theme.title('│'));
    console.log(theme.title('└──────────────────────────────────────────────────────┘'));
    console.log('');
...
export function showSetupGuide(): void {
    console.log('');
    console.log(theme.title('╔══════════════════════════════════════════════════════════════════════╗'));
    console.log(theme.title('║') + chalk.bold.white('              📧 Gmail Export - Setup Guide                          ') + theme.title('║'));
    console.log(theme.title('╚══════════════════════════════════════════════════════════════════════╝'));
    console.log('');

    console.log(theme.step('━━━ Step 1: Create a Google Cloud Project ━━━'));
    console.log('');
    console.log(`  ${theme.info('1.')} Go to: ${theme.highlight('https://console.cloud.google.com/')}`);
    console.log(`  ${theme.info('2.')} Click the project dropdown (top left) → ${theme.highlight('"New Project"')}`);
    console.log(`  ${theme.info('3.')} Name it something like ${theme.value('"Gmail Export"')} → Create`);
    console.log(`  ${theme.info('4.')} Wait for it to be created, then select it`);
    console.log('');
    ...
    console.log(`  ${theme.info('2.')} Search for ${theme.value('"Gmail API"')}`);
    console.log(`  ${theme.info('3.')} Click on it → Click ${theme.highlight('"Enable"')}`);
    console.log('');

    console.log(theme.step('━━━ Step 3: Configure OAuth Consent Screen ━━━'));
    console.log('');
    console.log(`  ${theme.info('1.')} Go to: ${theme.highlight('APIs & Services → OAuth consent screen')}`);
    console.log(`  ${theme.info('2.')} Select ${theme.value('"External"')} → Create`);
    ...
    console.log(`  ${theme.info('4.')} Click "Save and Continue" through the scopes page`);
    console.log(`  ${theme.info('5.')} ${theme.warning('⚠️  IMPORTANT:')} On "Test users" page:`);
    console.log(`     Click ${theme.highlight('"Add Users"')} and add ${theme.highlight('YOUR Gmail address')}`);
    console.log(`  ${theme.info('6.')} Save and Continue → Back to Dashboard`);
    console.log('');

    console.log(theme.step('━━━ Step 4: Create OAuth Credentials ━━━'));
    console.log('');
    console.log(`  ${theme.info('1.')} Go to: ${theme.highlight('APIs & Services → Credentials')}`);
    console.log(`  ${theme.info('2.')} Click ${theme.highlight('"Create Credentials"')} → ${theme.highlight('"OAuth client ID"')}`);
    console.log(`  ${theme.info('3.')} Application type: ${theme.value('"Desktop app"')}`);
    console.log(`  ${theme.info('4.')} Name: anything (e.g., "Gmail Export CLI")`);
    console.log(`  ${theme.info('5.')} Click "Create"`);
    console.log(`  ${theme.info('6.')} Click ${theme.highlight('"Download JSON"')} on the popup`);
    console.log(`  ${theme.info('7.')} Save the file as ${theme.highlight('credentials.json')} in your project folder:`);
    console.log(`     ${theme.dim(process.cwd() + '/credentials.json')}`);
    console.log('');

    console.log(theme.step('━━━ Step 5: Run the Export ━━━'));
    console.log('');
    console.log(`  ${theme.info('1.')} Make sure ${theme.highlight('credentials.json')} is in your project folder`);
    console.log(`  ${theme.info('2.')} Run: ${theme.highlight('npm run interactive')}`);
    console.log(`  ${theme.info('3.')} First time: a browser will open to authorize the app`);
    ...
    console.log(`  ${theme.info('4.')} A ${theme.highlight('token.json')} will be saved (don't commit this!)`);
    console.log(`  ${theme.info('5.')} Future runs won't need re-authorization`);
    console.log('');
    ...
    console.log(`  → Add yourself as a ${theme.highlight('Test User')} in OAuth consent screen`);
    console.log('');
    ...
    console.log(`  → Make sure you created a ${theme.highlight('"Desktop app"')} credential, not "Web"`);
    console.log('');
}
...
export function showFieldsList(): void {
    console.log('');
    console.log(theme.title('╔══════════════════════════════════════════════════════════════════════╗'));
    console.log(theme.title('║') + chalk.bold.white('              📋 Available Export Fields                            ') + theme.title('║'));
    console.log(theme.title('╚══════════════════════════════════════════════════════════════════════╝'));
    console.log('');

    const categories = ['sender', 'recipient', 'content', 'metadata', 'attachments'] as const;
    const categoryLabels: Record<(typeof categories)[number], {icon: string; name: string}> = {
    ...
    for (const category of categories) {
        ...
        for (const field of fields) {
            const defaultMarker = field.default ? theme.success(' (default)') : '';
    ...
    console.log(theme.title('──────────────────────────────────────────────────────────────────────'));
    console.log(`  Use ${theme.highlight('--fields')} to select specific fields:`);
    console.log(`  ${theme.dim('npx gmail-export --fields "from_email,subject,date,body_text"')}`);
    console.log('');
}
...
export function initConfigFile(): void {
    ...
    if (fs.existsSync(configPath)) {
        ...
        console.log(theme.dim('     Delete it first if you want to regenerate.\n'));
        return;
    }
    ...
    console.log(theme.dim('    Edit this file to customize your export settings.\n'));
}
