...
export function cleanText(s: string): string {
    return s
        ...
        .replace(/&#x([0-9a-fA-F]+);/g, (_, h) => {
            ...
            if (code < 32 || (code >= 0x200B && code <= 0x200F) || code === 0x34F || code === 0xFEFF || code === 0xAD) return '';
...
export function htmlToTextKeepingLinks(html: string): string {
    let s = html
        ...
        .replace(/(?:width|height|border|style|class|id|align|valign|bgcolor|cellpadding|cellspacing)\s*=\s*["'][^"']*["']/gi, ' ');
